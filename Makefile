...
db-init: docker-db
	...
	@sleep 3
	uv run --directory backend openclew db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend openclew db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend openclew db upgrade

db-downgrade:
	uv run --directory backend openclew db downgrade

db-current:
	uv run --directory backend openclew db current

db-history:
	uv run --directory backend openclew db history

# === Server ===
run:
	uv run --directory backend openclew server run --reload

run-prod:
	uv run --directory backend openclew server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend openclew server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend openclew user create-admin

user-create:
	uv run --directory backend openclew user create

user-list:
	uv run --directory backend openclew user list

# === Celery ===
celery-worker:
	uv run --directory backend openclew celery worker

celery-beat:
	uv run --directory backend openclew celery beat

celery-flower:
	uv run --directory backend openclew celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/openclew"
...
help:
	@echo ""
	@echo "openclew - Available Commands"
	@echo "======================================"
	@echo ""
	@echo "Setup:"
