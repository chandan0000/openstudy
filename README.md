...
Each generated project includes a CLI tool named `openclew`. Run commands from the `backend/` directory:

```bash
cd backend

# Server commands
uv run openclew server run --reload     # Start dev server
uv run openclew server routes           # Show all routes
# Database commands
uv run openclew db migrate -m "message" # Create migration
uv run openclew db upgrade              # Apply migrations
uv run openclew db downgrade            # Rollback migration
# User commands
uv run openclew user create-admin       # Create admin user
uv run openclew user create             # Create regular user
uv run openclew user list               # List all users
```
...
3. **Build and push Docker image**:
   ```bash
   docker build -t your-registry/openclew:latest ./backend
   docker push your-registry/openclew:latest
   ```
...
4. **Update image** in `kubernetes/kustomization.yaml`:
   ...
   images:
     - name: openclew
       newName: your-registry/openclew
       newTag: latest
   ```
...
kubectl scale deployment openclew-backend -n openclew --replicas=3

# View pods
kubectl get pods -n openclew

# View logs
kubectl logs -f deployment/openclew-backend -n openclew
```
...
# Server
uv run openclew server run --reload
uv run openclew server routes

# Database (Alembic wrapper)
uv run openclew db init
uv run openclew db migrate -m "Add users"
uv run openclew db upgrade

# Users
uv run openclew user create-admin       # Create admin (interactive)
uv run openclew user create             # Create user (interactive)
uv run openclew user list               # List all users
```
...
```bash
uv run openclew cmd seed --count 100
uv run openclew cmd seed --dry-run
```
...
| Start dev server | `uv run --directory backend openclew server run --reload` |
| Start prod server | `uv run --directory backend openclew server run --host 0.0.0.0 --port 8000` |
| Show routes | `uv run --directory backend openclew server routes` |
...
| Task | Command |
|------|---------|
| Create migration | `uv run --directory backend openclew db migrate -m "message"` |
| Apply migrations | `uv run --directory backend openclew db upgrade` |
| Rollback migration | `uv run --directory backend openclew db downgrade` |
| Show current | `uv run --directory backend openclew db current` |
| Show history | `uv run --directory backend openclew db history` |

### Users

| Task | Command |
|------|---------|
| Create admin | `uv run --directory backend openclew user create-admin` |
| Create user | `uv run --directory backend openclew user create` |
| List users | `uv run --directory backend openclew user list` |

### Celery

| Task | Command |
|------|---------|
| Start worker | `uv run --directory backend openclew celery worker` |
| Start beat | `uv run --directory backend openclew celery beat` |
| Start flower | `uv run --directory backend openclew celery flower` |
